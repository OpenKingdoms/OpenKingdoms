...
int Options_Tick(TAK_Platform *platform, float frame_dt) {
    ...
    } else if (opts.return_state == GAMESTATE_MENU) {
        SDL_Rect full = { 0, 0, 640, 480 };
        SDL_FillRect(off, &full, SDL_MapRGBA(off->format, 24, 24, 32, 255));
    }
    /* Opened from a battle or the lobby the panel is created in place
     * over it and paints no backdrop of its own (legacy:157830-157836,
     * legacy:137340-137347). */
