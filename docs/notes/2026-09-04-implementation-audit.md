...
1. **Sound events completely unwired**: GameSound_UnitAction/WeaponHit/PlayUI have zero call
...
2. **"node smoke" browser validation does not exist** (no node script in repo; build artifact
   real, dated Jul 13).
...
4. **No group selection**: Units_SelectSingle hard-sets g_selection_count=1; no marquee/shift/
   control groups. Blocks real play; blocks smoke §3/§6.
5. **Multiplayer menu is a dead-end screen**: ui/multiplayer.c has empty mp_routes[].
...
- units.c is 5,592 lines: sim + render not separated; it is the de facto simulation core.
- **AI calls Units_Command* directly — NOT on the command path lockstep needs.** Reroute AI
  through TAK_GameCommand before multiplayer.
...
Only 3 __EMSCRIPTEN__ ifdefs (all main.c); everything else gated by CMake/feature macros.
...
opposite is true) + redundant FS.mkdir('/game'). Browser build requires GOG install at
configure time (file(GLOB) over TAK_GAME_DIR).
