...
| KeyExists | :254086 |
| Key-table operations | :254097-254238. Get-key-by-index is a **positional blob stream**, not a lookup |
...
| Key | Value | Condition |
|---|---|---|
| `maxunits` | The FavoriteUnitsPerPlayer option, 20..500 | Always. Also the **only** back-compatibility probe on load, since older banks are detected by key absence |
...
dropped on purpose.** They must not survive a save/load round trip.
...
COB engine's serialised thread state (:228018-228020). That goes via an entry
point the reference tooling labels as a thread-killer, which is misleading,
since it is the serialiser. The state covers thread program counters, stacks,
locals, signals and sleeps. Without this a reload resumes every script from
the top.
...
- Back-compatibility is by key absence, and `maxunits` is the only probe.
...
- Lookup by name is at :221545.
- **Weighted variant pick** is at :221705. A CRT `rand()` draw is scaled by
...
- 2D play is at :221569. Action lookup is a linear string compare over the
  action list, and an **unknown action falls back to subclass 0** (:221615)
  rather than going silent.
- 3D play is at :221638.
...
1. Category is the low 3 bits of the flags word. The same value doubles as
   the Miles priority, so category and priority are never independent.
2. Loop flag is bit 5 of the flags word.
...
`0x1a5e0` = 108000 = 3600 × 30 (:21475), a `30/fps` expression at :243014, and
...
The net effect is at most 5 simulation ticks per render frame, plus a
fractional carry, plus hysteresis-driven automatic speed adjustment.
...
Once per **frame**, outside the tick loop: the AI batch pass, which is
**three** consecutive entry points *(tool-assigned auto-numbered method names
in the `BYMaia` cluster, meaningless as labels)*, then feature pool reset, LOS
...
Pause is bit 0 of the timing status bits, and the pacing function forces
`ticks = 0` while it is set. Render and input keep running.
...
All four of these bit-exactness requirements are load-bearing:
...
14. The `Celebrated` flag matters, and skipping it double-plays victory lines.
