...
description beside them does. The transcript in §1 is output from our own test harness.
...
Unit reached B, returned to A, reached B again, `cmd_kind` still PATROL — it loops
...
**(1a) Mission-scripted patrol is issued as a plain MOVE — moves once, stops.**
...
- src/render/units.c: add `Units_CommandPatrolUnit(int handle, int32_t x, int32_t y)`
  next to `Units_CommandMoveUnit` (:939-947) — same body but `cmd_kind=UNIT_CMD_PATROL`
  and `patrol_x/y = world_x/y` (mirror :973-977).
...
- src/ui/loading.c:211: give `MISSION_CMD_PATROL` its own case calling it
  (`cmd->a*16, cmd->b*16`). Multi-waypoint TDF patrols degrade to last-leg 2-point
  bounce — acceptable until an order queue exists; note in comment.
...
- `unit_on_damaged` units.c:500-518 (0537f7e): currently sets
  `victim->cmd_kind = UNIT_CMD_ATTACK` for PATROL victims — patrol lost. Same rule:
  when `cmd_kind == UNIT_CMD_PATROL` set only `victim->target` (+`unit_clear_path`);
  keep the ATTACK promotion for NONE.
- Melee kill clear units.c:3930-3936 (`fire_weapon_shot`): sets shooter
  `cmd_kind = UNIT_CMD_NONE` unconditionally when its target dies — kills patrol
  after one melee kill. Guard: only reset when `cmd_kind == UNIT_CMD_ATTACK`
  (projectile paths :605-650 don't touch the shooter — no edit there).
...
and stops (:4325-4327) — easily reported as "patrol/sweep moves once".
...
tester saw is **CLEAR** — its art is a broom (verified by decoding
...
refresh (:150749-150831 — the routine carries an address-suffixed automatic label, not a
real symbol): LOAD/UNLOAD by transport flag :150749-150775, HEAL :150776-150792, CLEAR
...
Exactly "Sweep shows the load/unload cursor".
...
8 CursorPatrol, 15 cursorrepair, 16 CursorUnload, 22 Cursorreclamate — matching
every `cursor_entry` in g_button_bindings. (The rendering dig's name list
:174EC-1753C is legacy handle-load order, not file order — don't "fix" indices.)
...
- src/ui/gui_render.c: add getter next to `GUIRuntime_SetWidgetVisible` (:427-435):
  ...
  stricmp scan, return `caches[i].hidden`; 0 when not found. Declare in
...
- src/ui/hud.c slot rebuild: after the `find_widget_rect` check (:726) add
  ...
  Only the visible member of each pair gets a slot; broom now arms `HUD_CMD_CLEAR`
  with Cursorreclamate. (Legacy pairs never conflict: transports get LOAD+UNLOAD,
  healers/reclaimers get HEAL+CLEAR — different rects.)
...
- Bottom bar: the HUD unit-info refresh at :152219 *(tool-assigned
  ...
  Experience; KillCount hidden :152496-152506; its aggregate-selection sibling at
...
- vis[] :552-584 gates `UnitMenu` (:578) and `CrystalBall` (:579) on
  ...
  Both wrong vs :151146-151152 (always visible).
- Action buttons: hidden when caps==0 — matches legacy idle. OK.
...
- Leave `HelpText`/`PositiveM`/`NegativeM`/BottomBar/EndCap/InfoPanel untouched
  (legacy whitelist :151146-151152; +N/−N refresh :701-709 already runs always).
...
- Sweep routing: unit test `GUIRuntime_WidgetHidden` (set visible 0/1 by name,
  ...
  then `HUD_HandleSidebarClick` at the CLEAR rect center scaled to window px —
