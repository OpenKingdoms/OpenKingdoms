...
Row index = (world Z ÷ 16) − (camera Y ÷ 16) + 32 — the object's map-cell row
minus the camera's, biased by 32 rows to keep the index positive (:210555).
Three parallel bucket lists are maintained side by side.
**There is no depth sort — Z-row bucketing only.**

## TA_SCALE — RESOLVED: 1 world unit = 1 screen pixel

- screen X = world X − camera X; screen Y = (world Z − world Y ÷ 2) − camera Y.
  No multiply anywhere in the path (:197195, :210761, :225772).
...
Terrain is drawn **flat** — there is no height displacement of tiles.
...
4. Two sine terms, each scaled by `d`: sine of (corner index + phase), and sine
   ...
   the reference tooling labels that second table a sin/cos lookup; here it is used as a
   per-index phase offset.
...
Gate: a global water-enable byte, and the renderer answering yes to a 3D
capability query (:225782). The software fallback (:225942) has **no water at
all** — plain tile blit with horizontal run merging. **No palette cycling.**
...
Why the legacy has no grid artifacts: half-resolution cells, a centre-aligned
lattice, per-vertex Gouraud interpolation, only three alpha levels, and
fully-visible quads skipped entirely.
...
- the unit definition does **not** carry flag bit 0x2000000 (name unrecovered —
  the bit value is the only handle we have on it; note it is the *same* bit
  that enables the 2× supersample path below, so the units that get AA are
  exactly the units that get no shadow);
...
the bilinear terrain height and the sea level — **the shadow sits on the
...
player's (or a cheat being active); skipped when health is below 1.
...
free-running frame counter) × 0x4000 ÷ 30 — a different global from the
simulation tick, so **the ring rotates with the frame counter**. Two arrays
step by ±0x2AAA with ∓0xCCC dash offsets. Vertices are rotated by the unit's
own orientation and projected through the same projector as the body.
...
(−health × 100 ÷ `maxdamage`; health is negative at death) plus the unit's
cached previous health percentage, halved, then clamped to 1..100. The COB
`Killed(severity, &corpsetype)` script runs unless the death cause forces a
...
type carries flag bit 0x1000000 (name unrecovered; the bit value is the only
handle on it).
...
⚠️ **DISCREPANCY.** This dig read 0x10082000 as `EMIT_SFX(sfxType, piece)`. Our
...
the VM — but re-verify the 0x1007x000 / 0x1008x000 host rows in a focused pass
...
**Effect asset table** (Visuals_Init :161407-161505) — global GAF handles for:
`bigsmoke`, `smoke`/`smoke01`, `flame`, `radiated`, `radiance02`, `radiance03`,
`radiance04`, `nobuild`, `nobuildx`, `mindspin`, `transportfx`, `transswirl`,
`deathmagic`, `purpledeath`, `pillaroflight`, `BlueFire`, `DieselFlame`,
`steam`; plus the health-bar strip, the 2×2 black and 2×2 white textures, and
the `cursors.gaf` frames: attack, airstrike, toofar, capture, defend, repair,
patrol, pickup, teleport, revive, reclamate, load, unload, move, select,
findsite, red, grn, normal, hourglass, pathicon.
...
**Global game-state block** — the globals this note refers to by name: map
width and height in cells; world width and height; viewport left/top/right/
bottom and viewport width/height; camera position; **sea level**; the explored
mask; the graphic tile map; the map-cell grid; the feature type table; the
corpse instance table; the scene Z-row buckets; the frame/tick counters; the
unit array; and the per-player records.
...
- The water-amplitude scalar and the phase-rate scalar are globals whose values
  ...
  matched.
- The concrete EmitSfx body is not recoverable from a data-only vtable —
...
1. **Fog overlay rewrite** (fully specified above — kills grid artifacts).
...
6. **SFX/EMIT host** (remaining unknown — vtable body unrecoverable; use
...
10. **TA_SCALE → 1 unit = 1 px (do FIRST — rescales everything).**
