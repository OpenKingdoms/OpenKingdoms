...
- There is no wait-for-all-peers barrier anywhere. The pacing function
  computes `lag = localFrame − oldestRemoteFrame` and scales the tick
...
The consequence is that TAK-RE multiplayer is a lockstep redesign, not a port.
Roughly 15 of the original's 45 message types exist purely to replicate state
...
**HAPINET context** is a single global structure holding, in order: the session
...
| :200218 | SendFrame is the pacing gate. If it has fallen behind, it resynchronises to `now + pacing / 2` rather than trying to catch up |
| :200320 | QueueMessage auto-flushes at **0x429 bytes** or **0x400 packets** |
| :200401 | NetBuffer::AddPacket has a maximum payload of **0x42A (1066) bytes** |
...
**NetMessage framing** comes in two shapes:
...
| 0x2B | ? | Damage, which feeds the damage application path |
...
**Determinism** is absent. The simulation PRNG is a Lehmer/MINSTD generator with
...
There is a 4-byte gap at 0x154 between the description and the build id, so the
...
**Step 0**: fix `THIRDPARTY_DIRECTPLAY.md` and `SPEC.md` §4.14 (six wrong
table entries).

**Step 1: transport abstraction.** `tak_net_transport.h`,
...
**Step 2: determinism substrate FIRST.** Reimplement the simulation PRNG and
...
is an intentional fix over the original, which never did. Audit every float and
...
**Step 3: command pipeline.** Slim `TAK_GameCommand` (currently a fixed
1044 bytes, so move to a variable unit list in an arena buffer). Add an
`execution_delay` of 3-5 frames at 30 Hz. CRC per serialised frame. Add a
`frame_hash` field to the buffer.

**Step 4: lockstep scheduler.** `TURN_LENGTH` of 4 ticks (~133 ms). Send the
...
**Step 5: session layer.** A 10-slot player table mirroring the legacy slot
...
**Step 6: desync detection** (the original has none). Every 30 ticks send
`{tick, SimHash}` reliably. On mismatch, freeze and dump the unit tables. **No
auto-recovery**, because a silent resync would hide the bug.

**Step 7: parity behaviours.** Chat bypasses the lockstep queue. Pause and
...
**Step 8 (only if needed)**: coalescing, a per-type size table, lz4 (skip the
