...
The local half of that checksum is in the tree. `TAK_SimHash` is FNV-1a over
the integer simulation state in a fixed order, and `TAK_SimHash_AI` is the
AI's share of it, its order matrices and its random cursor. `test_movement`
samples them every 60 ticks through the older names `Units_DebugStateHash`
