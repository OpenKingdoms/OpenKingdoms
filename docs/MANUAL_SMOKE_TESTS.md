...
- [ ] Map list populates, and picking maps updates the preview
- [ ] Change your faction, set opponent slot to AI, hit Play
- [ ] Loading bar advances through its stages and lands in-game without a stall
...
- [ ] Zoom around your castle: walls, keep, gate all render complete, with no
      missing walls or floating roofs
- [ ] Select a unit standing in the right third of the screen, then confirm it's
...
- [ ] Cancel: while a unit is mid-production, cancel it. The nanoframe vanishes
      and the next queued unit starts automatically
